...
def percusstion_test():
    bass_base = Subtraction(SineWaveOscillator(frequency=40.0),
                            Amplifeir(source=SquareWaveOscillator(frequency=80.0, duty=0.125),
                                      gain=1,
                                      attenuate=0.075))
    ...
    amplifire = Amplifeir(source=mixer, gain=Config.ValueRange[0], attenuate=1.0)
...
def chorus_test():
    ...
    amplifire = Amplifeir(source=mixer, gain=Config.MaxGain, attenuate=0.75)
    sink = WaveFileSink(output_file_name="output.wav")
